...
describe("Day 7: The Treachery of Whales", () => {
  ...
  const anotherFile = `${__dirname}/../data/test.txt`;
  ...
  test("Part One: check other data", async () => {
    const result = await doAlignCrabSubsPart1(anotherFile);
    const expectedResult = 328262;
    expect(result).toEqual(expectedResult);
  });

  test("Part Two: check other data", async () => {
    const result = await doAlignCrabSubsPart2(anotherFile);
    const expectedResult = 90040997;
    expect(result).toEqual(expectedResult);
  });  
});
